...
# for drug in approved_drugs4:
#     if drug['molecule_structures'] == None:
#        data = data.append({"Drug": drug['pref_name'], "Chemblid": drug['molecule_chembl_id'], "max_phase" : drug['max_phase'], "formula" : "-" , "synonyms" : drug['molecule_synonyms'], "canonical" : "-"}, ignore_index = True) 
       
#     else:
#        data = data.append({"Drug": drug['pref_name'], "Chemblid": drug['molecule_chembl_id'], "max_phase" :drug['max_phase'], "formula" :drug['molecule_properties']['full_molformula'] , "synonyms" : drug['molecule_synonyms'], "canonical" : drug['molecule_structures']['canonical_smiles']}, ignore_index = True)
...
for i in range(len(data.synonyms)):
    synonym=data.synonyms[i]
    
    for j in range(len(synonym)):
        d.append(synonym[j]['molecule_synonym'].upper())
        d.append(synonym[j]['synonyms'].upper())
        
    NewSynonyms.append(";;;".join(set(d)))
    d=[]
